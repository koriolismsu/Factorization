...
xi_minus_loop =[0:-step1:-1.5-step1,-1.5 - 1i*(0:step2:0.1-step2), -1.5-0.1i + (0:step1:1.5-step1), -1i*(0.1:-step2:0)];

xi_whole_loop = [-1.5:step1:1.5,1.5 + 1i*(0:step2:0.1),1.5 + 0.1i - (0:step1:3), -1.5 + 1i*(0.1:-step2:0)] - 0.04i;

xi_cur_loop = xi_plus_loop;

order = 1/3;

gamma_0 = sqrt(k^2  -  xi_cur_loop(1).^2);
root3_1 = (xi_cur_loop(1) + 1i*gamma_0)^(order);
root3_2 = (xi_cur_loop(1) - 1i*gamma_0)^(order);
...
covering_function = (root_cont(order,root3_1, xi_cur_loop + 1i*square_cont)+...
    root_cont(order,root3_2, xi_cur_loop - 1i*square_cont));%./square_cont;
